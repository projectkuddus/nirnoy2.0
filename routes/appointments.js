...
router.post('/appointments',needLogin,async(req,res)=>{
  ...
  // slot time fallback: today 18:00 + (serial-1)*dur minutes (simple MVP)
  const base='18:00'; // HH:MM
  function addMin(t,m){let [H,M]=t.split(':').map(Number),mins=H*60+M+m;H=Math.floor(mins/60)%24;M=mins%60;return `${String(H).padStart(2,'0')}:${String(M).padStart(2,'0')}`;}
